...
def simple_melee_weapon(x=0):
    ...
    if x==0:
        x, damage  = random.choice(list(simple_melee_weapons.items()))
    else:
        damage = simple_melee_weapons[x]
    return x,damage

#for simple ranged weapons 
def simple_ranged_weapon(x=0):
    #dictionary with weapon name attached with damage die and damage type
    simple_ranged_weapons = {'Light Crossbow':'1d8 piercing','Dart':'1d4 piercing','Shortbow':'1d6 piercing','Sling':'1d4 bludgeoning'}
    if x==0:
        x, damage = random.choice(list(simple_ranged_weapons.items()))
    else:
        damage = simple_ranged_weapons[x]
    return x, damage

#for martial melee weapons 
def martial_melee_weapon(x=0):
    ...
    if x==0:    
        x, damage = random.choice(list(martial_melee_weapons.items()))
    else:
        damage = martial_melee_weapons[x]
    return x, damage

#for martial ranged weapons
def martial_ranged_weapon(x=0):
